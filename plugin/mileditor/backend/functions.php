...
function set_subject()
{
    ...
    foreach ($results as $result) {
        ...
        //echo "console.log('name:".$name.", length:".$len_name."');";

        echo "var td = document.getElementById('$td_id');";
        echo "var bg = document.createElement('DIV');";
        echo "bg.className='table_bg';";
        //echo "var icon_box = document.createElement('DIV');";
        //echo "icon_box.className = 'icon_box';";
...
function modify_subject_in_milpage()
{
    ...
    $target_table = $wpdb->prefix . 'mil_subject_in_milpage';

    $result_ = $wpdb->update($target_table,
        array(
            'type' => $subject_type
        ),
        array(
...
add_action('wp_ajax_delete_subject_in_milpage', 'delete_subject_in_milpage');

/*********************
 * 교육과정에 포함되었던 과목의 정보를 삭제하는 함수
 *********************/

function delete_subject_in_milpage()
{
    global $wpdb;
    $result = array();
    ...
    $result_ = $wpdb->delete($table_name, array('page_id' => $page_id, 'subject' => $subject_code));
    ...
    if ($result_ == false) {
        ...
        $result['status'] = 'success';
        $result['message'] = __('삭제되었습니다.', 'mileditor');
    }
    header("Content-Type: application/json");
    echo json_encode($result);
    //echo $result;
...
function admin_coloring_suject()
{
    ...
    foreach ($results as $result) {
        ...
        switch ($subject_type) {
            case 'core':
                echo "jQuery('#" . $td_id . "').find('div').css({'background-color': '" . $color_core . "'});";
                break;
            case 'support':
                echo "jQuery('#" . $td_id . "').find('div').css({'background-color': '" . $color_support . "'});";
                break;
        }
    }
    ...
    foreach ($results_ as $result_) {
        ...
        if ($mandatory == 'T') echo "jQuery('#" . $td_id . "').find('div').css({'background-color': '" . $color_mandatory . "'});";
    }
    echo "</script>";
}
...
function mileditor_coloring_suject()
{
    ...
    foreach ($results as $result) {
        ...
        switch ($subject_type) {
            case 'core':
                ...
                echo "var icon = jQuery('<div>').appendTo('#" . $td_id . "').attr('id', '" . $td_id . "_icon');";
                echo "icon.addClass('type_icon');";
                echo "icon.css('backgroundColor', '" . $color_core . "');";
                echo "icon.css('borderColor', '" . $bordercolor_core . "');";
                echo "var text = document.createTextNode('C');";
                echo "icon.append(text);";
                break;
            case 'support':
                echo "jQuery('#" . $td_id . "').find('.table_bg').css({'background-color':'" . $color_support . "', 'border-color':'" . $bordercolor_support . "'});";
                echo "var icon = jQuery('<div>').appendTo('#" . $td_id . "').attr('id', '" . $td_id . "_icon');";
                echo "icon.addClass('type_icon');";
                echo "icon.css('backgroundColor', '" . $color_support . "');";
                echo "icon.css('borderColor', '" . $bordercolor_support . "');";
    ...
    $results_ = $wpdb->get_results("SELECT * FROM v_mil_position_mandatory");
    //$td_id_list = [];
    foreach($results_ as $result_) {
        $td_id = $result_->td_id;
        ...
        echo "var icon = jQuery('<div>').appendTo('#" . $td_id . "').attr('id', '" . $td_id . "_icon');";
        echo "icon.addClass('type_icon');";
        echo "icon.css('backgroundColor', '" . $color_mandatory . "');";
        echo "icon.css('borderColor', '" . $bordercolor_mandatory . "');";
        echo "var text = document.createTextNode('M');";
        echo "icon.append(text);";
        //array_push($td_id_list, '#' . $result_->td_id);
    }
    //$td_list_selector = "'" . join(', ', $td_id_list) . "'";

    //echo "jQuery(document).ready(function() {"."$(" . $td_list_selector . ").each(function(index, element) { stylizeMandatoryTd( element, '" . $color_mandatory . "', '" . $bordercolor_mandatory . "'); });})";
    echo "</script>";
}
...
function mil_update_subject()
{
    ...
    $subject_semester = $options['mil_subject_semester'];
    $subject_detail = $options['mil_subject_detail'];
    $subject_mandatory = !$options['mil_subject_mandatory'] ? '' : $options['mil_subject_mandatory'];
    $subject_tool = $options['mil_subject_tool'];
    $subject_presubject = $options['mil_subject_presubject_select'];
    $subject_relsubject = $options['mil_subject_relsubject'];
    ...
    if ($result_ == false) {
        ...
        $result['status'] = 'success';
        $result['message'] = __('설정하였습니다.', 'mileditor');
    }
...
add_action('wp_ajax_mil_delete_subject', 'mil_delete_subject');
/*********************
 * 교과목을 삭제하는 함수
 *********************/
function mil_delete_subject() {
    global $wpdb;
    $position_table = $wpdb->prefix . 'mil_position';
    $subject_table = $wpdb->prefix . 'mil_subject';
    $prereq_table = $wpdb->prefix . 'mil_prerequisite_subjects';
    ...
    $result_ = $wpdb->delete($subject_table,
        array(
            'code' => $subject_code,
            'name' => $subject_name,
            'detail' => $subject_detail,
